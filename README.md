...
Dimensions below come from the rebuilt meshes dated 2026-09-15. The assembly envelopes were checked. **The back cover uses a small removable support under the switch-well floor, included in the 3MF.** Its toolpaths now support the previously floating first contour. Remove this support before inserting the switch; physical hardware fit and support removal still need a print test. See the [current audit addendum](docs/hardware-measurements.md).
...
| `back` back cover | 1 | PETG white | 225 × 155 × 55.75 | outside on the bed, the three battery saddles and the hold-down plate stand upright; small local support under the switch-well floor |
...
Updated diagnostic estimate: **approx. 0.71 kg and 21.1 hours** for the four project plates, including their prime towers (housing plate 402 g / 10.66 h, back cover 167 g / 5.51 h, grey parts 129 g / 3.87 h, two TPU feet 13 g / 1.05 h). All seven individual parts and all four plates slice without CLI warnings. The switch support adds approximately 0.63 g and 1.4 minutes compared with the unsupported back cover. The separate single-part estimate in `docs/slicer-summary.json` totals 707.4 g / 21.1 h; [support verification](docs/switch-support-evidence-2026-09-15.json) records the final plate results and the corrected toolpaths.
...
| Potentiometer mounting | Local enclosure panel / nut thickness | 1.30 / 2.00 | Panel is a design choice; nut thickness remains an assumption. With a 3.60 mm thread, nominal remaining axial allowance is only 0.30 mm before any washer |
...
3. Slide the PWM module in from behind, potentiometer edge first, onto its supports above the electronics shelf; push the threaded section through the local 1.30 mm panel and tighten its nut outside after checking actual nut thickness and thread engagement. Fit the knob's circular, slotted sleeve onto the round splined shaft, with the pointer aligned to the desired control position; verify grip and removal on a fit sample. Glue the service cover in below it: a thin bead of 2K epoxy or CA gel in the groove of the side wall, press the cover in and hold it until the glue has set.
...
8. Solder about 15 cm of wire to + and − of the USB-C module, verify enough slack to lay the back cover aside, push the module into the channel on the inside of the back cover with the receptacle in the opening, and connect the wires through the lower cable notch in the partition to IN+ / IN− of the charge module. Remove the printed support through the large outside opening of the switch well; clean its floor and cut-out without levering against the thin 1.5 mm clip wall. Snap the measured power switch into its well at the upper right of the back cover from outside and wire it into the battery plus line; verify the revised wire route and slack with the cover open and closed. Put on the back cover (its three saddles close the cradle to rings with 0.5 mm nominal clearance around the cell body, the hold-down plate covers the back edge of the shelf) and screw it on with M3 × 8.

## Print and robustness

All parts except the back cover use no supports. The back cover has manual support painted only onto the eight downward-facing switch-floor triangles (148.19 mm²), using normal/manual, snug, build plate only, 0.20 mm top Z gap, three interface layers at 0.20 mm spacing, and 0.35 mm object XY clearance. These settings and the painted faces are preserved by the export script. **Use the 3MF for these support settings; an STL cannot store support painting.**

On the rebuilt meshes of 2026-09-15, `analyze.py islands`, `ridges` and `inlays` pass. The mesh-only overhang check still reports the switch floor (140.3 mm² at print height 7.1 mm); this geometry is now intentionally supported. In the sliced back-cover plate, the support interface ends at Z = 6.8 mm, 0.2 mm below the floor. The entire first contour has support beneath some portion of its extrusion width, removing the previous free-starting contour. Small gaps between support lines remain; this is not a solid contact surface. Verify support removal and switch clip fit on a PETG sample. Nothing has been printed yet. Smaller flagged features include the decorative grooves, screw-head pockets, knob flutes and connector openings.
...
- Potentiometer mount: the short 3.6 mm threaded section uses a local 1.3 mm wall with 45° transitions into the normal wall. The outer glue groove is omitted over this recess, with a matching interruption in the cover rim. A sampled ray check found a minimum 1.300 mm wall across the recess outside the shaft hole; this verifies geometry, not clamping strength.
...
- PWM/potentiometer: measure nut thickness and outside size across corners (currently assumed 2.00 mm and 11.00 mm), bare PCB thickness and underside components; confirm the approximate 6.00 mm axis height from the PCB top. Record the complete envelope with the fan connector plugged in and the terminal wires attached. Check the 3.60 mm threaded section with the intended 1.30 mm panel and actual nut/washer arrangement.
...
The local extension [`scripts/local_support.py`](scripts/local_support.py), called by `slice_check.py`, restores the switch support painting and per-object settings on each rebuild. It checks the selected triangle count and area, and slicing checks that support appears only on the back-cover plate.
