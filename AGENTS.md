...
- Back cover: rocker switch and USB-C socket in one column at x 165 (user). USB-C module below (z 52.5, between two battery saddles, just above the lower cable notch) in a channel, 1.5 mm skin for a flush receptacle; its stop is a 4 mm wedge from the partition above the lower cable notch (notch 10 mm high) that overlaps the left board edge by about 2.2 mm and ends 0.75 mm beside the battery removal path on the BMS side (the old right-wall stop is out of reach there); for this the battery moved from x 180 to 183 and the feet from 17 to 15 mm inset (foot bosses beside the battery); rocker switch low beside the PWM module above the battery (z 101; user: lower, grip above it) in an 8 mm deep 45° well (rocker 1 mm below the back face), floor 1.5 mm, 0.2 mm floor margin (no support needed), well wall 2.2 mm horizontal = 1.56 mm across the flank.
