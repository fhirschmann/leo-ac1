...
- Back cover: USB-C module low right (x 205, z 44) in a channel, 1.5 mm skin for a flush receptacle, 4 × 6 mm wedge stop on the right wall; rocker switch low beside the PWM module above the battery (x 167, z 101; user: lower, grip above it) in an 8 mm deep 45° well (rocker 1 mm below the back face), floor 1.5 mm, 0.2 mm floor margin (no support needed), well wall 2.2 mm horizontal = 1.56 mm across the flank.
...
- Bail: source the Ø 7 × 6.9 mm sleeves (bore ≥ 4.2); check the Ruthex RX-M4x8.1 datasheet (hole 5.6 taken from the leoino case, length 8.1 and wall 2.2 assumed); 0.5 mm gaps between arm, step wall and back cover (pinch check on the print).
