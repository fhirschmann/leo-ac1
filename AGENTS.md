...
- TPU feet screwed (2 × M3 × 8 each) into outside-pressed inserts; L-shaped folding bail like the user's leoino case (user: L for more hand room, folds back): upper legs in steps (15 × 12 mm) along both top side edges from the pivot to the back edge, side walls full height in front; lower legs and grip sunk flush into a recess of the back cover (user; legs 0.5 mm below the back face): the recess is a shell of the back cover reaching 13 mm into the housing inside its walls (walls 2.4, bridges over the openings print on the back face; `bail_recess_shell/void/room()`), so the housing side walls stay whole (user: no seam or step on the side); the housing is cut back only inside (step floor near the back, partition) and the back intake slots keep clear of the shell; the lower legs are 8.15 mm wide (outer shell wall inside the side wall); 50 mm finger scoop under the middle of the grip (41 degree slope, user: the flush grip was hard to get out); 1 mm chamfers on the recess edges at the back face and where the legs pass the step floor (the chamfer hulls must start from a thin slab at the face, a deep box turns them into long slopes); grip centred at z 108 (user: lower grip keeps about 40 mm hand room, the shorter reach of the sunk legs would otherwise leave 29 mm); carried at bail_carry (140.7°, grip above the pivots, fan level), stopped by a ramp at the front of each step (the upper leg face touches the chamfered ramp at bail_carry, 0.015 mm gap; contact check `bail_up@body` along the leg face normal — the old clearance offset left 0.49 mm and 3 degrees of overtravel); 1 mm chamfers on the step edges (top edges of ramp and inner wall as one hull so they meet cleanly; also in the back-cover corners); bail built from fully chamfered boxes and a chamfered eye (`chamfered_box`); step shell clipped to the rounded outer outline; 40.4 mm hand room, 196 mm grip; pivot stack measured from the leoino STLs (step 12, arm 11, eye 8.8 flush with the inner arm face, bore Ø 7.0, Ruthex M4 hole Ø 5.6 × 8.1), then step and arm 3 mm wider outwards (user: sink the screw heads): step 15 mm, arm 14 mm with 0.5 mm to side face and wall, outer 5.2 mm of the arm cut back round the eye, shoulder screw head 0.5 mm below the arm face (assert); top back-cover bosses moved inwards beside the steps, back-cover lip and corners cut for the legs; swing sampled at half and full carrying angle as extra assembly bodies; printed with the back faces of the lower legs on the bed; swing the bail up before the back cover comes off (back_off path); the bail comes off backwards after its screws (bail_off path); M5 insert in the underside.
...
- Accepted bridges: `overhangs back` reports the 14 mm (bar) and 11.8 mm (legs) bridges that close the bail recess (about 3300 mm², test_right_back about 1200 mm²); both ends are supported, the slicer prints them without supports.
