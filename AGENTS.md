...
- QR code to https://github.com/fhirschmann/leo-ac1 on the back cover (user): grey inlay in the first 0.6 mm of the back face, version 3-M, 1 mm modules (dark modules grown 0.05 per side so corner-touching modules do not make degenerate points), centre x 188, z 97 with a 4-module quiet zone between the intake slots, USB-C socket and the folded grip; `qr_code.py` (segno) writes the module grid into the `// BEGIN qr` block, rerun it when the URL changes. Checked by rasterising `back_qr.stl` as seen from behind and decoding it (OpenCV), orientation matches the reference matrix.
